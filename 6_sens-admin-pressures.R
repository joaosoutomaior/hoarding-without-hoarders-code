...
#==============================================================
# contents: 
#  - sensitivity for administrative pressures
...
dat <- read_csv("../data/sens-admin-pressures/results.csv") %>%
...
# prepare plot
dat_plot <- dat_plot %>%
  group_by(admin_pressures) %>%
  summarise(RATIO = value, .groups = "drop")

# ggplot
p <- dat_plot %>%
  ggplot() +
  geom_smooth(aes(x = admin_pressures,
                 y = RATIO),
              method = 'loess',
              color = "red",
              size = 0.5,
  ...
  scale_y_continuous(expression("simulated racial penalties, " * theta[1]),
                     limits = c(-1,0.5),
                     expand = c(0,0)) +
  scale_x_continuous(expression("salience of administrative pressures, " * beta["admin-pressures"]),
                     expand = c(0,0)) +
...
# save
ggsave("figures/fig_sens-admin-pressures.png", width = 9, height = 9, units = "cm", dpi = 1000)
