...
#==============================================================
# contents: 
#  - sensitivity for variance in the error term for academic background
...
dat <- read_csv("../data/sens-acad-error-variance/results.csv") %>%
...
dat_base <- dat %>%
  select(run_id, net_diffusion, acad_error_variance) %>%
  dplyr::distinct()
...
dat_plot <- dat_plot %>%
  mutate(net_diffusion = as.factor(net_diffusion),
         acad_error_variance = as.factor(acad_error_variance))

# compute means
mu <- dat_plot %>%
  group_by(net_diffusion, acad_error_variance) %>%
  summarise(group_mean = mean(value, na.rm = TRUE), .groups = "drop")

# labeller function
epsilon_labeller <- as_labeller(function(x) {
  paste0("epsilon[1] %~% N(0, ", x, ")")
...
p <- ggplot() +
  ...
  facet_wrap(~acad_error_variance, nrow = 1, labeller = epsilon_labeller) +  
  ggtheme_facet +
  theme(
    legend.position = "top",
...
# save
ggsave("figures/fig_sens-acad-error-variance.png", width = 17, height = 8, units = "cm", dpi = 1000)
